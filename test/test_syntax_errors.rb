...
class TestTemplateSyntaxErrors < Minitest::Test
  ...
  def test_end_tag_mismatch
    source = "{% if true %}foo{% endfor %}"
    message = "unexpected tag \"endfor\""
    error = assert_raises(Luoma::TemplateSyntaxError) { Luoma.render(source) }
    assert_equal(message, error.message)
  end
  ...
  def test_invalid_bracketed_path
    source = "{{ foo[1.2] }}"
    message = "expected an integer, identifier or string"
    error = assert_raises(Luoma::TemplateSyntaxError) { Luoma.render(source) }
    assert_equal(message, error.message)
  end
  ...
  #   source = "{{ {{a: 2} }}"
  #   message = "expected a string or identifier"
  #   error = assert_raises(Luoma::TemplateSyntaxError) { Luoma.render(source) }
  #   assert_equal(message, error.message)
  # end
